...
export const run = async () => {
  ...
  */
  ...
  const responsePrompt = await fewShotPrompt.format({ country: 'France' });
  ...
  // We can the pass fewShots prompt to the model to answer
  const model = new OpenAI({ temperature: 0.9 });

  //Calls out to the model's (OpenAI's) endpoint passing the prompt. This call returns a string
  const res = await model.call(responsePrompt);
